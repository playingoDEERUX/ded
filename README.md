...
The way it works: N amount of deterministic domains are generated every month. These are probed first, assuming no endpoint is discovered there, it'll scan further with an endpoint list based on a hardcoded FIRST_OCTET using a /8 IP pool (the fallback).
